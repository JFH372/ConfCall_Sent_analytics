...
#Requesting html
html3 <- read_html("C:/Users/jhasl/OneDrive/Documents/Text Analysis/Project_SWT/Q32019ResultsSA.html")


#Reading the body of the html, and converting it to a readable text format
transcript_textQ3 <- html_text(html_nodes(html3, "#a-body"))

#Seperating the text by new line characters in html code
transcript_textQ3 <- strsplit(transcript_textQ3, "\n") %>% unlist()


#Remove empty lines
transcript_textQ3 <- transcript_textQ3[!stri_isempty(transcript_textQ3)]

#Getting the earnings date
earnings_dateQ3 <- html_text(html_nodes(html3, "time")) %>% paste0(collapse = "")
...
transcript_textQ3 <- gsub("–","-",transcript_textQ3)
...
idx_eQ3 <- min(which(str_detect(transcript_textQ3, "[[:upper:]][\\w]+ -")))
...
transcript_textQ3 <- transcript_textQ3[idx_eQ3:length(transcript_textQ3)]
idx_eQ3 <- 1

#Repeating to find the starting index for the analyst names
idx_aQ3 <- min(which(!str_detect(transcript_textQ3, "[[:upper:]][\\w]+ -")))
...
execQ3 <- transcript_textQ3[idx_eQ3:(idx_aQ3-1)]
execQ3 <- str_match(execQ3, pattern1)
execQ3 <- execQ3[1:nrow(execQ3),2]
execQ3 <- append(execQ3, "Olivier Le Peuch")
...
idx_oQ3 <- min(which(!str_detect(transcript_textQ3, "[[:upper:]][\\w]+ -"))[-1]) - 1
analystQ3 <- transcript_text[(idx_aQ3+1):idx_oQ3]
analystQ3 <- str_match(analystQ3, pattern1)
analystQ3 <- analystQ3[1:nrow(analystQ3),2]
analystQ3 <- append(analystQ3, "Operator")
